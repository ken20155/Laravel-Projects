  ...
  foreach ($result as $key => $R) {
    ?>
      <tr>
        ...
        <td><?=$R->business_approved_msme != null ? $R->business_approved_msme : $R->business_approved ?></td>
        <?php
        ...
        <?php
          ...
          if ($R->status_msme == 'PENDING') {
            ...
            ?>
                <td>
                  <button type="button" class="btn btn-success openFormBnrMsme" data-action="edit" data-id="<?=$R->msme_id?>" data-status="<?=$R->status_msme?>" data-form="msmeprinted" style="width:100%"><i class="fas fa-file-alt"></i> VIEW</button>
                </td>
            <?php
          }
        }
