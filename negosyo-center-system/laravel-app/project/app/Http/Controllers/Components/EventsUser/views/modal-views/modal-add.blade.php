...
$status_participant = $participants ? $participants->status : 'PENDING';
$reason_cancel_participant = $participants ? $participants->reason_cancel : '';
...
<div class="row">
    ...
    <div class="col-md-4">
        <label for="validationCustom01" class="form-label">Event Category</label>
        <select name="event_category" id="" class="form-select form-select-sm" required>
            <option value="" disabled selected>-Select-</option>
            <option value="Micro" <?= $event_category == 'Micro' ? 'selected' : ''?>>Micro</option>
            <option value="Small" <?= $event_category == 'Small' ? 'selected' : ''?>>Small</option>
            <option value="Medium" <?= $event_category == 'Medium' ? 'selected' : ''?>>Medium</option>
        </select>
    </div>
    <div class="col-md-4">
    ...
    <div class="col-md-12 <?= $status == 'CANCELLED' ? '' : 'd-none' ?>">
        <label for="validationCustom01" class="form-label">Event Reason for cancellation</label>
        <textarea  class="form-control form-control-sm" cols="30" rows="3" readonly><?= $reason_cancel ?></textarea>
    </div>
    <div class="col-md-12 <?= $status_participant == 'CANCELLED' ? '' : 'd-none' ?>">
        <label for="validationCustom01" class="form-label">Your Reason for cancellation</label>
        <textarea  class="form-control form-control-sm" cols="30" rows="3" readonly><?= $reason_cancel_participant ?></textarea>
    </div>
    <?php } ?>
