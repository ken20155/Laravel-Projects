<?php 
namespace App\Http\Controllers\Components\AppointmentScheduleUser;
use App\Models\MainModel;
use App\Http\Controllers\Helpers\Helpers;

class AppointmentScheduleUser 
{
    ...
    function __construct() {
        ...
        Helpers::getView('AppointmentScheduleUser');
    }
    ...
    public function list(){
        ...
        $result =  $this->model->getResults($sql, [$this->session['user_id'],'Appointment']);  
        $data = [];
        $x = 1;
        if ($result) {
            foreach ($result as $key => $R) {
                ...
                if ($R->event_status == 'CANCELLED') {
                    ...
                    $final_status = $R->status_participant;
                }
                ...
                $data[] = [
                 ...
                 'type' => 'Appointment',
                 'status' => Helpers::status_badge($final_status,$status),
    ...
    public function createDocument($post){
        ...
        if ($res) {
            ...
            $where = [
                ['field' => 'event_id','value'=> $post['selected_event_id']]
            ];
    ...
    public function viewEvents($post){
        extract($post);
        $sql = 'SELECT * FROM tbl_events WHERE deleted = ? and `type` = ? ';
        $param = [0,'Appointment'];
        ...
        if ($data) {
            ...
            foreach ($data as $key => $R) {
                ...
                $array_data[] = [
                    'id'    =>  $R->event_id,
                    'title' => 'STATUS: '.$status.'<br>
                    TIME START: '.Helpers::timeFormat($R->start_time).'<br>
                    TIME END: '.Helpers::timeFormat($R->end_time).'<br>
                    '.$extend_msg.'
                    ',
    ...
    public function openDynamicModalOther(){
        ...
        return [
            ...
            'footer' => $d['data']->status == 'CANCELLED' ? '' : '<button type="button" data-id = "'.$id.'" id = "selectSchedule" class="btn btn-primary btn-sm">Select</button>',
        ];
    }
