...
class AuthController extends Controller
{
    ...
    public function signin(){
        ...
        //dd(bcrypt($password));
        $sql = 'SELECT * FROM tbl_users WHERE username = ? and `status` = ?';
        $param = [$username,'ACCEPTED'];
        ...
        if ($result && Hash::check($password,$result->password)) {
            ...
            $session = [
                ...
                'user_role_desc' => $result->user_role == 1 ? 'ADMIN' : 'BUSINESS OWNER',
            ];
    ...
    public function signup(){
        ...
        if ($result) {
            ...
            $data['street'] = $street;
            $data['form_ownership'] = $form_ownership;
            $data['category_entrepreneur'] = $category_entrepreneur;
            $data['bday'] = $bday;
            $data['email'] = $email;
