<?php 
namespace App\Http\Controllers\Components\PurchaseRequest2;
use App\Models\MainModel;
use App\Http\Controllers\Helpers\Helpers;
class PurchaseRequest2 
{
    function __construct() {
        $this->model = new MainModel();
        $this->module = Helpers::last_segment();
        Helpers::getView('PurchaseRequest2');
    }
    ...
    public function addNew(){
        ...
        $sql2 = "SELECT * FROM tbl_clients WHERE deleted = 0";
        $d['clients'] =  $this->model->getResults($sql2, []); 
        $d['data'] = false;
        return [
            'action' => 'Add New',
            'body' =>  view('modal-content.view', $d)->render(),
    ...
    public function supplierItems(){
        $post = Helpers::post();
        extract($post);
        $sql2 = "SELECT * FROM tbl_supplies WHERE deleted = 0";
        $d['data'] =  $this->model->getResults($sql2, []); 
        return [
            'body' =>  view('modal-content.supplier', $d)->render(),
        ];
    }
}