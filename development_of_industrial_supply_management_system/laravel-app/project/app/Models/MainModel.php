...
class MainModel extends Model {
    ...
    public function batchInsertUpdate($table, $unique_key, $records){
        $firstRecord = $records[0] ?? [];
        $allColumns = array_keys($firstRecord); // Get all column names from the first record
        $updateColumns = array_diff($allColumns, $unique_key); 
        DB::table($table)->upsert($records, [$unique_key], $updateColumns);
    }
}
