                    ...
                    <div class="col-4">
                        ...
                        <div class="input-group bootstrap-touchspin bootstrap-touchspin-injected">
                            ...
                            <span class="input-group-btn input-group-prepend">
                                <button type="button" class="btn btn-primary bootstrap-touchspin-down increaseDecrease" data-cartid="<?= $R->cart_id ?>"  data-type="dec" data-qty="<?= $R->qty ?>"   data-stocks="<?= $total_stocks ?>"type="button">-</button>
                            </span>
                            
                            <input id="demo2" type="text" value="<?= $R->qty ?>" data-cartid="<?= $R->cart_id ?>" name="produdct_details[<?=$R->cart_id?>][qty]" class="form-control form-con text-center getDataSold input-pos-qty" style="width: 50px; text-align: center;" data-stocks="<?= $total_stocks ?>">
                            ...
                            <input type="hidden" name="produdct_details[<?=$R->cart_id?>][product_name]" value="<?= $R->product_name ?>" class="getDataSold" id="">
                            
                            <span class="input-group-btn input-group-append">
                                <button type="button" class="btn btn-primary bootstrap-touchspin-up increaseDecrease" data-cartid="<?= $R->cart_id ?>" data-type="inc" data-qty="<?= $R->qty ?>"  data-stocks="<?= $total_stocks ?>" type="button">+</button>
                            </span>
...
<div style="border: 1px solid rgb(35, 27, 27);padding:5px;">
    ...
    <input type="hidden" name="grand_total" value="<?= $grand_total ?>" class="getDataSold" id="grand_total">
    
    <div class="row">
        <div class="col-6">Enter Amount</div>
        <div class="col-6"><input type="text" name="input_amount" id="input_amount" value="" class="form-control form-control-sm text-right getDataSold" ></div>
        <div class="col-6">Change</div>
        <div class="col-6"><input type="text" name="change_amount" id="change_amount" value="" class="form-control form-control-sm text-right getDataSold"  readonly></div>
    </div>
</div>
<div class="row mt-3 mb-2">
    <div class="col-6"><button type="button" class="btn btn-danger resetNow">Reset</button></div>
    <div class="col-6 d-none"><button type="button" class="btn btn-primary testPrint ">Test</button></div>
    <div class="col-6 text-right"><button type="button" class="btn btn-success distributeNow" disabled>Submit</button></div>
</div>
<?php 
    }else{
