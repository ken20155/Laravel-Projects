...
class PosSystem
{
    ...
    public function generateStockProducts(){
        ...
        WHERE a.deleted = 0 and c.stock_id is not null and a.total_stocks > 0 $where_ex";
    ...
    public function generateStockProductsSelected(){
        ...
        LEFT JOIN (SELECT SUM(qty) as qty_sold,stock_id FROM tbl_product_transaction WHERE deleted = 0 GROUP BY stock_id) as f ON a.stock_id = f.stock_id
        LEFT JOIN (SELECT SUM(qty_return) as qty_return,stock_id FROM tbl_return_logs WHERE deleted = 0 GROUP BY stock_id) as e ON a.stock_id = e.stock_id  
        WHERE a.deleted = 0 and a.added_by = ? and b.total_stocks > 0 ";
    ...
    public function increaseDecreaseInput(){
        $post = Helpers::post();
        extract($post);
    ...
    public function distributeNow(){
        $post = Helpers::post();
        extract($post);
        $data_1['receipt_no'] = strtotime(date(SQLDATETIME));
        $data_1['total'] = $grand_total;
        $data_1['input_amount'] = $input_amount;
        $data_1['change_amount'] = $change_amount;
        $data_1['added_date'] = date(SQLDATETIME);
        $session = $this->session;
        $data_1['added_by'] = $session['user_id'];
        $res = $this->model->insert('tbl_transaction_receipt', $data_1);
        $parent_id = $res['id'];
        $data_2 = [];
        foreach ($produdct_details as $key => $R) {
            $data_2[] = [
                ...
                'added_date'    =>$data_1['added_date'],
                'added_by'      =>$session['user_id'],
            ];
        }
        $this->model->batchInsert('tbl_product_transaction',$data_2);
        if ($res) {
            $response['html'] = $this->printReceipt($data_1,$produdct_details);
            $response['success'] = true;
            $response['msg'] = 'Purchased Successfully!';
        }else{
            $response['success'] = true;
            $response['msg'] = 'Purchase Failed!';
        }
        return $response;
    }
    public function printReceipt($data_1,$produdct_details){
        $d['test'] = '';
        $d['detail'] = $data_1;
        $d['product'] = $produdct_details;
        return view('product-content.receipt', $d)->render();
    }
