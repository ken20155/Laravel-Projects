$(document).ready(function() {
    ...
    $(document).on('change', '.input-pos-qty', function() {
        ...
        request.done(function(response) {
            generateStockProductsSelected(false); 
            //alert(response.qty_max)
            if (response.qty_max) {
                main.msg('Exceed stock quantity!');
            }
            //$.unblockUI();
        });
    });
    ...
    $(document).on('click', '.distributeNow', function() {
        ...
        main.confirmation(title,function () {
            ...
            var data = $('.getDataSold').serialize();
            var request = main.form_ajax(url, data, 'POST', function(xhr) {
                main.block_ui();
            });
            request.done(function(response) {
                if (response.success) {
                    ...
                    main.printDirect(response.html);
                }else{
    ...
    $(document).on('click', '.testPrint', function() {
        var url = getRunURL('printReceipt');
        var data = $('.getDataSold').serialize();
        var request = main.form_ajax(url, data, 'POST', function(xhr) {
            main.block_ui();
        });
    ...
    $(document).on('input', '#input_amount', function() {
        ...
        if (inputAmount < grandTotal) {
            ...
            $('.distributeNow').prop('disabled',false);
        }

        let changeAmount = inputAmount - grandTotal;
