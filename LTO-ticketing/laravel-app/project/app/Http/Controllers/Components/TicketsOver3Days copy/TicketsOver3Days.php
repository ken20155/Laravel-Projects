<?php 
namespace App\Http\Controllers\Components\TicketsOver3Days;
use App\Models\MainModel;
use App\Http\Controllers\Helpers\Helpers;
use Illuminate\Support\Facades\Hash;
class TicketsOver3Days 
{
    function __construct() {
        $this->model = new MainModel();
        $this->module = Helpers::last_segment();
        Helpers::getView('TicketsOver3Days');
    }
    ...
    public function listUsers(){
        ...
        if (isset($post['start']) && isset($post['end'])) {
            $where_ext .= ' and a.added_date between "'.$post['start'].'" and "'.$post['end'].'"';
        }
        $sql = "SELECT a.*,concat(b.first_name,' ',b.middle_name,' ',b.last_name) as added_by_name FROM tickets as a inner join tbl_users as b on a.added_by = b.user_id WHERE a.deleted = 0 and a.`status` = 'Pending' and DATEDIFF(CURDATE(), a.added_date) >= 3 $where_ext ORDER BY a.added_date DESC";
        ...
        if ($result) {
            ...
            >
             ...
             $btn_paid = '<button
                ...
                ';
                $my_btn = $btn_view.$btn_pending.$btn_paid;
                }else{
                    $my_btn = $btn_view;
                    $status = '<span
